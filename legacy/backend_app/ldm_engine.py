...
# Checkpoint search paths (newest first). DEVGEN_CONTROLNET_DIR overrides —
# point it directly at a folder containing controlnet/config.json + weights
# (used on Kaggle where the checkpoint is an attached input).
...
_ENV_CONTROLNET_DIR = os.environ.get("DEVGEN_CONTROLNET_DIR")
...
def _find_controlnet_checkpoint():
    ...
    """
    if _ENV_CONTROLNET_DIR:
        cn_dir = _ENV_CONTROLNET_DIR
        if os.path.exists(os.path.join(cn_dir, "config.json")):
            print(f"[LDM Engine] Using DEVGEN_CONTROLNET_DIR: {cn_dir}")
            return cn_dir
        raise FileNotFoundError(f"DEVGEN_CONTROLNET_DIR has no config.json: {cn_dir}")
...
def load_ldm_pipeline(checkpoint_path=None):
    ...
    # runwayml repo was taken down; fall back to the official mirror
    pipe = None
    for model_id in ("runwayml/stable-diffusion-v1-5", "stable-diffusion-v1-5/stable-diffusion-v1-5"):
        try:
            ...
            print(f"[LDM Engine] Could not load base '{model_id}': {exc}")
    if pipe is None:
        raise RuntimeError("No Stable Diffusion v1.5 base model reachable.")
    pipe = pipe.to(device)
...
def generate_handwriting_ldm(text: str, checkpoint_path=None, font_path="font.ttf",
                             conditioning_scale=1.0, seed=42, num_inference_steps=25):
    """
